...
import { DEFAULT_FOUNDATION_MODEL_ID, DEFAULT_PREDEFINED_FOUNDATION_MODEL_LIST, FoundationModelIds } from '../demo/infra/src/application/ai/foundation-models/ids';
...
(async () => {
  ...
  const execCommand = (...args: Task): ReturnType<typeof execa.execaCommand> => {
    lastTasks.push(args);
  ...
  const cached = <T extends Record<string, any>>(answers: T, prefix?: string): T => {
  ...
  console.log(chalk.cyanBright(figlet.textSync(NAME, { horizontalLayout: "fitted" })));

  console.log(chalk.magentaBright("Galileo companion cli for deploying the cdk infra"));
  ...
  program
    .version("0.0.0")
    .description("Galileo helpers for common tasks");
  ...
  program
    ...
    .option("--cdk-cmd <value>", "CDK command to run; defaults to deploy", "deploy")
    .option("--no-projen", "Skip projen")
    .option("--yes", "Skip confirmations", false)
    .option("--last", "Replay last successful task(s) execution", false)
    .option("--save", "Save successful task(s) execution to enable last replay", true)
  ...
  if (options.last) {
    ...
    console.info(commandMessage("LAST", "Replay the last task(s):", _lastTasks.map((_task) => {
      return chalk.gray("→ ") + _task[0];
    }).join("\n")));

    if (options.yes || (await prompts({
      type: "confirm",
      name: "confirmed",
      message: "Execute?"
    })).confirmed) {
  ...
  const selectedFoundationModels = new Set<FoundationModelIds>(cache.getItem("foundationModels") || DEFAULT_PREDEFINED_FOUNDATION_MODEL_LIST);
  ...
  const {
    ...
    deploySample,
    foundationModels,
  } = cached(await prompts([
    {
      type: "text",
      name: "profile",
      message: "AWS Profile",
      initial: options.profile || cache.getItem("profile") || process.env.AWS_PROFILE,
    ...
    },
    {
      ...
      initial: options.region || cache.getItem("region") || process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION,
      validate: async (value: string) => (value && value.length > 0) || "Region is required",
    },
    {
      type: "text",
      name: "adminEmail",
      message: "Administrator email address" + chalk.reset.grey(" Enter email address to automatically create Cognito admin user, otherwise leave blank\n"),
  ...
  const {
  ...
  } = cached(await prompts([
    {
      ...
      choices: (foundationModels as string[]).map((x) => ({ title: x, value: x })),
      initial: () => {
        const _initial = cache.getItem("defaultModelId") ?? DEFAULT_FOUNDATION_MODEL_ID;
    ...
    },
    {
      ...
      initial: cache.getItem("deployModels") ?? DeployModelOptions.SAME_REGION,
      choices: [
        { title: "Yes, in same region as application", value: DeployModelOptions.SAME_REGION },
        { title: "Yes, but in different region", value: DeployModelOptions.DIFFERENT_REGION },
        { title: "No, already deployed", value: DeployModelOptions.ALREADY_DEPLOYED },
        { title: "No, but link to cross-account stack", value: DeployModelOptions.CROSS_ACCOUNT },
  ...
  const account = (await execa.execaCommand(`aws --profile ${profile} sts get-caller-identity --query Account --output text`)).stdout;
  ...
  const crossAccountRegex = new RegExp(`arn:aws:iam::\\d{10,12}:role\\/${applicationName}-FoundationModel-CrossAccount-\\w+`);
  ...
  switch(deployModels) {
    ...
    case DeployModelOptions.DIFFERENT_REGION: {
      const { modelRegion } = cached(await prompts({
        type: "text",
        name: "modelRegion",
        message: "What region do you want to deploy Foundation Models to?",
        initial: cache.getItem("modelRegion") ?? region,
        validate: async (value: string) => (value && value.length > 0) || "Model region is required",
      }, { onCancel }));
      ...
      const { modelRegion } = cached(await prompts([
        {
          type: "text",
          name: "modelRegion",
          message: "What region was the Foundation Model stack deployed to?",
    ...
    case DeployModelOptions.CROSS_ACCOUNT: {
      const { modelRegion, crossRegionRoleArn } = cached(await prompts([
        {
          type: "text",
          name: "modelRegion",
          message: "What region was the Foundation Model stack deployed to in other account?",
        ...
        },
        {
          ...
          validate: async (value: string) => (value && crossAccountRegex.test(value)) || `Invalid cross-account arn - expected "${crossAccountRegex.source}"`,
        },
      ], { onCancel }));
      context.set("DecoupleStacks", "1");
      context.set("FoundationModelRegion" , modelRegion);
  ...
  cached(await prompts<any>([...new Set<string>([region, modelRegion]).values()].map((_region) => ({
    type: "confirm",
  ...
  if (regionsToBootstrap.size > 0) {
    const { cloudformationExecutionPolicies } = cached(await prompts({
      type: "text",
      ...
      initial: cache.getItem("cloudformationExecutionPolicies") ?? "arn:aws:iam::aws:policy/PowerUserAccess,arn:aws:iam::aws:policy/IAMFullAccess",
    }, { onCancel }))

    const bootstrapCmd = `cdk bootstrap --profile ${profile} ${[...regionsToBootstrap].map((r) => `aws://${account}/${r}`)} --cloudformation-execution-policies "${cloudformationExecutionPolicies}"`
    const confirmed = options.yes || (await prompts({
      type: "confirm",
      name: "confirmed",
      message: commandMessage("[CDK BOOTSTRAP]", `Execute the following command in ${account}?`, bootstrapCmd),
      initial: true,
    }, { onCancel })).confirmed;
    if(confirmed) {
  ...
  if (options.yes || (await prompts({
    type: "confirm",
    name: "confirmed",
    message: commandMessage(`CDK ${options.cdkCmd.toUpperCase()}`, `Execute the following command in ${account}?`, cmd),
    initial: true,
  }, { onCancel })).confirmed) {
    ...
    options.build && await execCommand("pnpm build", {
      cwd: path.join(ROOT),
      stdio: "inherit",
    });
...
function commandMessage(context: string, description: string, cmd: string): string {
  return chalk.cyanBright(`[${context}]`) + " " + description + "\n" + chalk.magentaBright(cmd) + "\n";
}

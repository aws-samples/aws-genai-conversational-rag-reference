...
import { deleteChatHandler, DeleteChatResponseContent } from 'api-typescript-runtime';

const dynamodb = new DynamoDBClient({});
const documentClient = DynamoDBDocumentClient.from(dynamodb);

export const handler = deleteChatHandler(...interceptors, async ({ input, interceptorContext }) => {
  ...
  if (!chatId) {
    throw new Error(`no chat id in request parameters`);
  }

  await deleteChatAndMessages(documentClient, tableName, indexName, userId, chatId);

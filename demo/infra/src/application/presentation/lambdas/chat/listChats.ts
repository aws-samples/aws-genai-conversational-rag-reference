...
import { listAllChatsByTime } from "@aws-galileo/galileo-sdk/lib/chat/dynamodb/lib/chat";
...
export const handler = listChatsHandler(
  ...interceptors,
  async ({ interceptorContext }) => {
    ...
    const items = await listAllChatsByTime(
      documentClient,
      tableName,
      indexName,
      userId

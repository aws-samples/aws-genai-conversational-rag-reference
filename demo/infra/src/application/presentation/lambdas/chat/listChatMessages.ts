...
import { parseNextToken, generateNextToken } from '@aws/galileo-sdk/lib/chat/dynamodb/lib/util';
import { DynamoDBClient } from '@aws-sdk/client-dynamodb';
import { DynamoDBDocumentClient } from '@aws-sdk/lib-dynamodb';
import { interceptors } from 'api-typescript-interceptors';
import { ChatMessage, ListChatMessagesResponseContent, listChatMessagesHandler } from 'api-typescript-runtime';

const dynamodb = new DynamoDBClient({});
const documentClient = DynamoDBDocumentClient.from(dynamodb);

export const handler = listChatMessagesHandler(...interceptors, async ({ input, interceptorContext }) => {
  ...
  const newNextToken = queryResults.LastEvaluatedKey ? generateNextToken(queryResults.LastEvaluatedKey) : undefined;

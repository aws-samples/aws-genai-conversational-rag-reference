...
import { UpdateChatResponseContent, updateChatHandler } from 'api-typescript-runtime';

const dynamodb = new DynamoDBClient({});
const documentClient = DynamoDBDocumentClient.from(dynamodb);

export const handler = updateChatHandler(...interceptors, async ({ input, interceptorContext }) => {
  ...
  const result = await updateChat(documentClient, tableName, userId, chatId, title);

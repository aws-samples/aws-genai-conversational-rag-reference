...
import { listChatMessageSources } from '@aws/galileo-sdk/lib/chat/dynamodb/lib/sources';
...
export const handler = listChatMessageSourcesHandler(...interceptors, async ({ input, interceptorContext }) => {
  ...
  const Items = await listChatMessageSources(documentClient, tableName, userId, messageId);

...
export class ApplicationStage extends Stage {
  constructor(scope: Construct, id: string, props?: ApplicationStageProps) {
    ...
    if (config.rag.samples?.datasets.includes(SampleDataSets.SUPREME_COURT_CASES)) {
      ...
      const sampleDatasetStack = new SampleDatasetStack(this, `${applicationName}-SampleDataset`, {
      ...
      NagSuppressions.addStackSuppressions(sampleDatasetStack, [
        {
          ...
          reason: 'stack has no control of custom resource lambda version used by SampleBucketDeployment',
        },
        {
          id: 'AwsPrototyping-IAMNoWildcardPermissions',
          reason: 'stack has no control of permissions used by SampleBucketDeployment',
        },
        {
          id: 'AwsPrototyping-IAMNoManagedPolicies',
          reason: 'stack has no control of policies used by SampleBucketDeployment',
        },
      ]);
    }
    ...
    Aspects.of(this).add({
      visit: (node) => {
        if (node instanceof lambda.Function) {
          ...
          Object.entries(ApplicationContext.getPowerToolsEnv(node)).forEach(([key, value]) =>
            node.addEnvironment(key, value),
          );
        }
      },
    });
    Aspects.of(this).add(new lambda.FunctionVersionUpgrade(LAMBDA_RECOGNIZE_LAYER_VERSION));
  }
}

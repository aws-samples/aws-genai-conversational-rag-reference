...
export class IndexingPipeline extends Construct {
  ...
  constructor(scope: Construct, id: string, props: IndexingPipelineProps) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    const sagemakerSecurityGroup = new SecurityGroup(this, 'SagemakerSecurityGroup', {
      allowAllOutbound: true,
      vpc,
    });
    sagemakerSecurityGroup.addIngressRule(sagemakerSecurityGroup, Port.allTraffic());
    NagSuppressions.addResourceSuppressions(
      sagemakerSecurityGroup,
      [
        {
          id: 'AwsPrototyping-EC2RestrictedCommonPorts',
          reason: 'SecurityGroup granting access to itself only and is in VPC, so all ports are ok for development',
        },
        {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    const vectorStoreSetupTaskLambda = new NodejsFunction(this, 'VectorStoreSetupTaskLambda', {
    ...
    const vectorStoreSetupTask = new tasks.LambdaInvoke(this, 'VectorStoreSetupTasK', {
    ...
    const shouldRunProcessingJobCheck = new Choice(this, 'ShouldRunProcessingJobCheck', {
      comment: 'Should run processing job?',
    })
      .when(Condition.booleanEquals(StatePaths.DoRunProcessingJob, true), runProcessingJobChain)
      .otherwise(cancelState);
      ...
      [
        {
          ...
          reason: 'StateMachine lambdas use default managed policy AWSLambdaBasicExecutionRole',
        },
        {
          id: 'AwsPrototyping-IAMNoWildcardPermissions',

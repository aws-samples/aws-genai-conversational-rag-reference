...
export class GalileoCdk extends AwsCdkConstructLibrary {
  constructor(monorepo: MonorepoTsProject) {
    ...
    NxProject.ensure(this).addBuildTargetFiles(undefined, ['{projectRoot}/assets']);
    ...
    const autoDiscoverComps = this.components.filter((v) => v instanceof AutoDiscoverBase) as AutoDiscoverBase[];
    autoDiscoverComps.forEach((c: AutoDiscoverBase) => {
      if (c instanceof LambdaAutoDiscover) {
        c.entrypoints.forEach((entrypoint) => {
          const basePath = path.posix.join(path.dirname(entrypoint), path.basename(entrypoint, '.lambda.ts'));

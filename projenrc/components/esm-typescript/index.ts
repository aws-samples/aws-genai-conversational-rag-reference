...
export interface EsmTypescriptProjectOptions
  extends Omit<TypeScriptProjectOptions, keyof typeof PROJECT_AUTHOR | 'defaultReleaseBranch'> {
  /**
...
export class EsmTypescriptProject extends TypeScriptProject {
  constructor(options: EsmTypescriptProjectOptions) {
    ...
    // @ts-ignore - read-only
    this.tsconfigDev.exclude = this.tsconfigDev.exclude.filter((v) => !['**/*.spec.*', '**/*.test.*'].includes(v));

...
export class NxPythonProject extends PythonProject {
  ...
  constructor(options: NxPythonProjectOptions) {
    ...
    if (!(this.root instanceof MonorepoTsProject)) {
      throw new Error(
        "NxPythonProject must be nested in root MonorepoTsProject"
      );
    }
    const monorepo = this.root as MonorepoTsProject;
    ...
    options.implicitDependencies &&
      this.nx.addImplicitDependency(...options.implicitDependencies);

    const dockerDistDir = (this.dockerDistDir =
      options.dockerDistDir || ".docker-dist");
    ...
    this.packageTask.exec(
      `mkdir -p ${dockerDistDir} && rm -rf ${dockerDistDir}/*`
    );
    this.packageTask.exec(
      `rsync -a --exclude=*__pycache__* ${this.moduleName} ${dockerDistDir}/`
    );
    this.packageTask.exec(
      `poetry export --without-hashes --format=requirements.txt > ${dockerDistDir}/requirements.original.txt`
    );
    this.packageTask.exec(
      `sed -n '/file:\\/\\//!p' ${dockerDistDir}/requirements.original.txt > ${dockerDistDir}/requirements.txt`
    );

    options.relativePoetryDependencies &&
      options.relativePoetryDependencies.forEach((dep) => {
        if (dep.tryFindFile("pyproject.toml") == null) {
          throw new Error(
            `[NxPythonProject] All 'relativePoetryDependencies' must be poetry python projects: ${dep.name}`
          );
        }
        monorepo.addPythonPoetryDependency(this, dep);
        const moduleDir = path.join(
          path.relative(this.outdir, dep.outdir),
          dep.moduleName
        );
        this.packageTask.exec(
          `rsync -a --exclude=*__pycache__* ${moduleDir} ${dockerDistDir}/`
        );
      });
    ...
    if (options.dockerfile !== false) {
      ...
      const from =
        options.dockerFrom ??
        `public.ecr.aws/lambda/python:${dockerPythonVersion}`;
      const handler = options.dockerHandler || "handler.handler";
      const handlerMethod = path.extname(handler).slice(1);
      const handlerBase = handler.replace(
        new RegExp(`\.${handlerMethod}$`),
        ""
      );
      ...
      this.dockerfile.addLine("");
      this.dockerfile.addLine(
        `RUN python${dockerPythonVersion} -m pip install -r requirements.txt`
      );
      this.dockerfile.addLine("");
      this.dockerfile.addLine("# Test importing of the handler");
      this.dockerfile.addLine(
        `RUN python${dockerPythonVersion} -c "import os; os.environ['PYTHON_ENV']='test'; from ${this.moduleName}.${handlerBase} import ${handlerMethod}; print('success')"`
      );
      this.dockerfile.addLine("");

...
export namespace accountUtils {
  ...
  export const retrieveCdkBootstrapInfo = async (options: CdkBootstrapInfoRequestOptions) => {
    const CACHE_KEY = `bootstrapinfo-${options.region}`;
    // if profile is default --> always check
    // otherwise - cache
    if (options.profile !== 'default') {
      const regionBootstrapCached = context.cache.getItem(CACHE_KEY) as CdkBootstrapInfo;
...
export { CdkBootstrapInfo, CdkBootstrapInfoRequestOptions } from './get-bootstrap-info';

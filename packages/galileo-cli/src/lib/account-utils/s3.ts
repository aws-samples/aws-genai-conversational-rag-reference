...
import * as path from 'node:path';
import { GetBucketTaggingCommand, ListBucketsCommand, S3Client, S3ServiceException } from '@aws-sdk/client-s3';
...
export namespace s3 {
  export const listBuckets = async (credentials: CredentialsParams): Promise<string[]> => {
    ...
    context.ui.spinner.text = 'Loading buckets (filtering index processing buckets...)';
      ...
      try {
        ...
        if (
          containsAppComponentTag(
            tagsResp.TagSet?.map((t) => <Tag>{ key: t.Key!, value: t.Value! }) ?? [],
  ...
  export const uploadDocuments = async (options: UploadDocumentsRequest) => {
    ...
    if (path.isAbsolute(rootDir)) {
      ...
      if (options.metadataFilepath) {
        rootDirAbs = path.resolve(path.join(path.dirname(options.metadataFilepath), rootDir));
      } else {
    ...
    for (let i = 0; i < docKeys.length; i++) {
      ...
      if (docValue.pageContent) {
        ...
        if (fileExt !== 'txt') {
          console.warn(chalk.yellowBright(`File ${docKey} is not a txt file. Skipping...`));
          continue;
        }

        if (!fs.existsSync(docFilepath)) {
          console.warn(chalk.yellowBright(`File ${docFilepath} doesn't exist. Skipping...`));
          continue;
        }

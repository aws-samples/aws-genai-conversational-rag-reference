...
export const loadMetadataAndValidate = (filePath: string): DocumentMetadata => {
  // load schema
  const schemaContent = fs.readFileSync(path.join(__dirname, 'metadata.schema.json'), { encoding: 'utf-8' });
...
export const loadDocumentMetadata = async (loaderOrMetaFilePath: string): Promise<LoadDocumentMetadataResult> => {
  ...
  if (ext === '.json') {
    ...
    try {
      ...
      throw new DocumentMetadataLoaderError('Error loading metadata from external module', {
        cause: err,
      });
    }
  }
